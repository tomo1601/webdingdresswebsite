...
public class DetailCartDAO {
    ...
    public List<DetailCart> getDetailCartById(String cid) {
        Query query = em.createQuery("SELECT p FROM DetailCart p WHERE p.detailCartPK.iDCart = :iDCart");
    ...
    public List<DetailCart> searchByIdProd(String cid, String pid) {
        Query query = em.createQuery("SELECT p FROM DetailCart p WHERE p.detailCartPK.iDCart = ?1 and p.detailCartPK.iDProd = ?2");
